...
---
See http://datascienceplus.com/imputing-missing-data-with-r-mice-package/
...
```
The variable modelFit1 containts the results of the fitting performed over the imputed datasets, while the pool() function pools them all together. Apparently, only the Ozone variable is statistically significant. Note that there are other columns aside from those typical of the lm() model: fmi contains the fraction of missing information while lambda is the proportion of total variance that is attributable to the missing data. For more information I suggest to check out the paper cited at the bottom of the page.
...
```
```{r}
#head(tempData2, n=10) produces way too much oputput - do not know why
...
##A Missing Data Function

```{r missingDataFunction}
